...
\fancyfoot[C]{Unified NTT RTL 사례로 배우는 area와 timing 설계}
...
\large Unified NTT RTL 사례로 배우는 Area와 Timing 설계}
...
\vspace{0.8cm}
{\Large Unified NTT RTL 사례로 배우는 Area와 Timing 설계\par}
\vspace{1.0cm}
...
\begin{center}{\large 합성 보고서에서 출발해 RTL 구조로 해결하는 설계 기록}\end{center}
...
이 책은 문법별 장난감 예제를 나열하는 대신, 하나의 실제 설계가 100~MHz에서
150~MHz로 발전한 과정을 따라간다. 사례는 radix-2, radix-3, trinomial 변환을
지원하는 Unified NTT 가속기이며, 비교 대상은 다음 세 시점이다.
...
각 사례는 \textbf{관찰한 보고서}, \textbf{RTL에서 찾은 원인},
\textbf{선택한 구조}, \textbf{검증해야 할 계약}, \textbf{측정 결과} 순으로
설명한다. 코드 조각은 이해에 필요한 부분만 줄여 싣고, 정확한 신호 연결과
파라미터는 위 RTL 파일을 기준으로 한다.
...
\input{chapters/01_hardware_view}
\input{chapters/02_measurement}
\input{chapters/03_architecture}
\input{chapters/04_timing_cases}
\input{chapters/04a_frequency_journal}
\input{chapters/05_area_cases}
\input{chapters/05a_operator_cost}
\input{chapters/06_arithmetic}
\input{chapters/07_schedule_interface}
\input{chapters/08_verification}
\input{chapters/09_workflow}
\input{chapters/10_results}

\appendix
\input{chapters/a_glossary}
\input{chapters/b_references}
